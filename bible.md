...
## 12. 상태 및 생명주기 관리 (State & Lifecycle Management)

안전한 운영을 위해 봇과 서비스의 **상태 전이(State Transition)**는 명확하고 안전해야 한다.

1.  **중간 상태 활용 (Intermediate States)**
    - 즉각적인 종료(`STOPPED`)보다는 정리 작업을 위한 중간 상태(`STOPPING`)를 활용한다.
    - 예: 사용자가 Stop을 요청하면 `STOPPING`으로 전이 → 포지션 청산 등 정리 작업 수행 → `STOPPED`로 최종 전이.

2.  **Graceful Shutdown (우아한 종료)**
    - 모든 서비스/봇은 종료 시 **소유한 리소스(DB 연결, 열린 포지션, 파일 핸들 등)를 스스로 정리**해야 한다.
    - 강제 종료(`Kill`)는 시스템 장애 상황을 제외하고는 지양한다.

3.  **상태의 단일 진실 공급원 (SSOT for State)**
    - 봇의 상태(`status`)와 자산(`position/balance`) 정보는 **DB 또는 원장(Ledger)**을 진실 공급원으로 삼는다.
    - 메모리 상태에 의존하지 않고, 항상 영속화된 데이터를 기준으로 의사결정을 내린다.