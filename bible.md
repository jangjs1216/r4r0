...
### 4.1 데이터 필드 명명 규칙 (Golden Rule of Naming)

키 불일치 오류를 방지하기 위해 다음 규칙을 엄격히 준수한다.

1.  **No Aliases (별칭 금지)**:
    - Pydantic의 `Field(..., alias="name")` 사용을 **엄격히 제한**한다.
    - **내부 변수명 = DB 컬럼명 = JSON Key = Frontend 인터페이스**가 되도록 이름을 통일한다.
    - 예: `schema_def` (내부) -> `schema` (JSON) 처럼 이름을 꼬지 말고, 그냥 내부에서도 `schema`로 쓴다.

2.  **명시적 매핑 (Explicit Mapping)**:
    - 불가피하게 이름이 달라야 한다면(예: legacy DB), 자동 변환에 의존하지 말고 **명시적인 변환 로직**을 작성한다.
    - AI는 코드 생성 시 Frontend의 인터페이스(`interface`)와 Backend의 응답(`ResponseModel`) 필드가 정확히 철자까지 일치하는지 **스스로 교차 검증**해야 한다.
...
6. **임시 코드 및 스크립트 관리**
   - 개발 및 검증 과정에서 생성된 **일회성 스크립트(예: 마이그레이션, 임시 테스트)**는 목적 달성 후 **즉시 정리**한다.
   - **DB 마이그레이션 스크립트**: DB 구조 변경이 완료되면 삭제하거나, 별도의 `migrations/` 아카이브 폴더로 이동시킨다. (프로젝트 루트에 방치 금지)
   - **검증용 테스트 스크립트**: 비즈니스 로직 검증에 유효한 코드라면, 일회성으로 쓰지 말고 **정식 테스트 코드(`tests/`)로 리팩토링**하여 영구적으로 관리한다.
...
- 주요 테스트 시나리오 및 검증 방법
- 테스트 실행 명령어 (예: `pytest ...`)
- 핵심 비즈니스 로직(예: PnL 계산) 검증 스크립트 위치
...
## 11. 의존성 관리 원칙 (Dependency Management)

마이크로서비스라 하더라도, 유지보수성과 호환성을 위해 **의존성 버전 관리**에 주의를 기울인다.

1.  **버전 고정 (Pinning)**
    - 모든 `requirements.txt` 또는 `package.json`의 의존성은 **명시적인 버전 번호**를 가진다(예: `fastapi==0.109.0`).
    - `latest`나 버전 명시 없는 설치는 피한다.

2.  **버전 일관성 (Consistency)**
    - 서로 다른 서비스에서 **동일한 라이브러리**를 사용할 경우, 가능한 한 **동일한 버전**을 사용한다.
    - 예: `AuthService`에서 `FastAPI 0.109.0`을 쓴다면, `BotService`도 특별한 이유가 없다면 `0.109.0`을 맞춘다.

3.  **최소 의존성 (Minimalism)**
    - 해당 서비스의 목적에 꼭 필요한 라이브러리만 설치한다.
    - 편의성만을 위해 무거운 라이브러리를 추가하는 것을 지양한다.

---

## 12. 주석 및 언어 원칙 (Comment and Language Principles)

1. **주석의 한국어 원칙**:
   - 모든 코드 내 주석, Docstring, 설명문은 **한국어로 작성**하는 것을 원칙으로 한다.
   - 이는 사람(사용자/개발자)이 AI의 산출물을 더 빠르고 정확하게 검증하고, 비즈니스 의도를 명확히 파악하며, 유지보수성을 극대화하기 위함이다.
2. **설명의 구체성**:
   - 복잡한 비즈니스 로직, 상태 전이(State Transition), 또는 아키텍처적인 결정 사항이 있는 경우, 코드 주석을 통해 그 이유와 배경을 상세히 기술한다.
3. **용어의 일관성**:
   - `ServiceSpec.md`나 도메인 모델에서 정의한 용어를 주석에서도 동일하게 사용하여 혼선을 방지한다.
