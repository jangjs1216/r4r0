...
export const BacktestResultDashboard: React.FC<Props> = ({ result }) => {
    return (
        <div className="flex flex-col h-full gap-6 animate-in fade-in duration-500">
            ...
            <div className="flex-grow min-h-[450px] bg-gray-950/50 rounded-3xl border border-gray-800/80 p-6 shadow-2xl relative overflow-hidden group">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <MetricCard
                    label="Net Profit"
                    value={`+$${result.metrics.totalPnl.toLocaleString()}`}
                    subValue=""
                    icon={DollarSign}
                    trend="up"
                ...
                <MetricCard
                    ...
                    subValue={`${result.metrics.totalCost > 500 ? 'High Activity' : 'Low Activity'}`}
                    icon={Percent}
                    trend="up"
            ...
            <div className="bg-gray-900/30 border border-gray-800 rounded-xl p-4 flex items-center justify-between text-sm text-gray-400">
                <div className="flex gap-6">
                    <div>
                        ...
                        <span className="font-mono text-gray-200">+${result.metrics.alpha.toLocaleString()}</span>
                    </div>
                    <div className="h-8 w-px bg-gray-800"></div>
                    <div>
                        <span className="block text-xs uppercase text-gray-600 font-semibold mb-1">Total Cost</span>
                        <span className="font-mono text-red-400">-${result.metrics.totalCost.toLocaleString()}</span>
                    </div>
                </div>
                <div className="flex gap-4 opacity-70">
                    <span className="text-xs">Fees: <b>${result.costBreakdown.fees}</b></span>
                    <span className="text-xs">Slippage: <b>${result.costBreakdown.slippage}</b></span>
                    <span className="text-xs">Funding: <b>${result.costBreakdown.funding}</b></span>
                </div>
            </div>
